...
class _VoiceTranscriptBody(BaseModel):
    transcript: str


@router.put("/voices/{filename}/transcript")
async def save_voice_transcript(filename: str, body: _VoiceTranscriptBody) -> dict:
    """Persist a reference transcription as a sidecar next to the WAV."""
    async with httpx.AsyncClient(timeout=5.0) as client:
        try:
            r = await client.put(
                f"{DUBBING_URL}/voices/{filename}/transcript",
                json={"transcript": body.transcript},
            )
        except httpx.HTTPError as exc:
            raise HTTPException(status_code=502, detail=f"dubbing-generator: {exc}")
    if r.status_code >= 400:
